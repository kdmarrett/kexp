...
def recordTrial(wheel_matrix_info, preblock, id_, wav_indices, instr, ec, el, stimdir, final_datadir, record_pupil=True):
	...
	assert((trial_vars['paradigm'][0].encode('ascii') == id_), 'Error: id_ and paradigm from mat file do not match')
	...
	for i in range(len(trial_vars['possible_letters'])):
		letter = trial_vars['possible_letters'][i][0][0].encode('ascii')
		if (letter == target_letter):
			color = 'Lime'
		else:
			color = 'LightGray'
		txt_obj.append(ec.screen_text(letter, pos = letter_loc[0][i], color=color, font_size=18))
	ec.flip(primer_length)
