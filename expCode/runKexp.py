...
def recordTrial(wheel_matrix_info, preblock, id_, stim_trial_indices, instr, ec, el, stimdir, final_datadir, record_pupil=True):
	...
	for i in range(len(wheel_matrix_info)):
		...
		temp = CircularLayout(
			letters_wheel, radius=.15, relative_center=wheel_loc.positions[i])
	...
	# Refer to voc_meg.py and pupillometry.py and PylinkAPI.pdf for referenc
	# on recording pupillometry
	pdb.set_trace()
	...
	for snum in range(len(block_in_sections)):
		...
		for bnum in range(block_in_sections[snum]):
			...
			for tnum in range(trial_in_block[snum]):
				...
				stim_trial_indices = recordTrial(
					wheel_matrix_info, preblock, id_, stim_trial_indices, instr, ec, el, stimdir, final_datadir, record_pupil)
