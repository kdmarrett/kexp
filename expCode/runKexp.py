...
def recordTrial(wheel_matrix_info, preblock, id_, wav_indices, instr, ec, el, stimdir, final_datadir, record_pupil=True):
    ...
    ec.wait_secs(3)  # +++ speeds debugging
    ec.stop()
    ec.clear_buffer()
...
def cogLoadSurvey(gen_survey, mid_survey, rel_survey, id_, ec):
    ...
    for qnum in dict.keys(gen_survey):
        ...
        surveyInput(gen_survey[qnum], ec)
    for qnum in dict.keys(mid_survey):
        screen_text(mid_survey[qnum])
        ec.flip()
        response = wait_one_press(live_keys=survey_btn)
        ec.flip()
        ec.write_data_line(response)
    for qnum in dict.keys(rel_survey):
        ec.write_data_line('rel_survey')
        ec.write_data_line(id_ + 'Q_no_' + str(qnum))
        ec.screen_text(rel_survey[qnum])
        ec.flip()
        response = wait_one_press()
        ec.flip()
        # response = get_keyboard_input(rel_survey[qnum], out_type=int)
        ec.write_data_line(response)

def surveyInput(text, ec):
    while not correct_input:
        screen_text(text)
        ec.flip()
        response = wait_one_press()
        feedback = "You pressed" + str(response) + "if this the number you want press "{}" to continue otherwise press the number that you intended"
        ec.flip()
        if response in survey_btn:
            if 
...
with ef.ExperimentController(*std_args, **std_kwargs) as ec:
    # import ipdb; ipdb.set_trace()
    stimdir = op.join(PATH, 'Stims', ec._exp_info['participant'], ec._exp_info['session'])
    ...
    final_datadir = op.join(datadir, ec._exp_info['participant'], ec._exp_info['session'])
    ...
    for snum in [2]: # range(len(block_in_sections)):
        ...
        for bnum in range(block_in_sections[snum]):
            ...
            for tnum in range(trial_in_block[snum]):
                ...
                if snum == 2:
                    ...
                    trial_key = 's' + str(0) + '_' + 'start_trial_' + str(condition_no)
                else:
                    record_pupil = True
                    trial_key = 's' + str(snum) + '_' + 'start_trial_' + str(condition_no)
                # start trial
                # import ipdb; ipdb.set_trace()
                ec.screen_prompt(
