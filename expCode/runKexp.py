...
instr['s1_start_sect'] = ('In this experiment you will be listening to letters grouped into'
	  ...
	  ' will appear in the center of screen.  Look at the dot as soon as possible after it appears and move your eyes as little'
...
instr['s1_end_trial'] = ('Good work! You\'re done with this trial.'.format(cont_btn_label))# replace cont_btn label with wait dur
...
instr['s3_end_trial'] = ('Thank you for you responses.') #continue automatically
...
mid_survey[1] = 'The procedure is simple: You will be presented with a series of pairs of rating scale titles (for'
...
def recordTrial(stim_trial_indices, instr_key, condition_binary, record_pupil = True, trial_target_times, possible_letters, wheel_matrix_info, ec, trial_path)
	...
	for snum in len(section)
		...
		for bnum, block in section[snum]:
			...
			for tnum in section[snum][bnum] # +++ check this 
				...
				if (snum == 2):
					record_pupil = False
				recordTrial(stim_trial_indices, instr_key, condition_binary, record_pupil, trial_target_times, possible_letters, wheel_matrix_info, ec, trial_path)	
				if (snum == 2):
