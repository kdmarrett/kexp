function [ dropped_matrix, target_index, target_in_wheel ] = generateDroppedSlots( letters_matrix, target_letter, target_per_block, oddball_drops )
...
if ~oddball_drops
    dropped_matrix_components = ones(m, n); %deletes non target oddballs
end
...
for i= 1:length(row_indices)
    dropped_matrix_init(i, :) = dropped_matrix_components(row_indices(i), :); % fill in those set rows with non target components
end
...
row_indices = randperm(rep_block, rep_block); %get random ordering of the rows
